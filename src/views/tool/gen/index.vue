    ...
    <el-dialog :title="preview.title" v-model="preview.open" width="80%" top="5vh" append-to-body>
      <el-tabs v-model="preview.activeName">
        <el-tab-pane
          ...
          :key="key"
        >
        <pre v-highlightjs><code class="highlightedLanguage(value, key)" v-html="highlightedCode(value, key)"></code></pre>
        </el-tab-pane>
      </el-tabs>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    highlightedCode(code, key) {
      ...
      // const result = hljs.highlightBlock(language, code || '', true, {})
      // return result.value || '&nbsp;'
      return code
    },
    /** 获取语言 */
    highlightedLanguage(code, key) {
      const vmName = key.substring(key.lastIndexOf('/') + 1, key.indexOf('.vm'))
      const language = vmName.substring(vmName.indexOf('.') + 1, vmName.length)
      return language
    },
